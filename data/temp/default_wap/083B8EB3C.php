...
<!-- 容器 -->
<div class="weui-tab">
    <div class="weui-navbar"> <?php unset($k, $v); $k_index = 0; foreach ((array)$car_type as $k => $v): $k_index ++;?> <a class="weui-navbar__item <?php if ($k == 1):?>weui-bar__item--on<?php endif;?>" data-cid="<?php echo($k);?>" href="#tab<?php echo($k);?>"> <?php echo($v);?> </a> <?php endforeach;?> </div>

    <div class="weui-tab__bd"> <?php unset($k, $v); $k_index = 0; foreach ((array)$car_type as $k => $v): $k_index ++;?> <div id="tab<?php echo($k);?>" class="weui-tab__bd-item <?php if ($k == 1):?>weui-tab__bd-item--active<?php endif;?>">
...
var cars = $.parseJSON('<?php echo($cars);?>');

var order_car_url = "<?php echo(RGX\router::url('order-preview-cid-~')); ?>";
$.each (cars , function (ctype , item) {
    ...
    $.each(item , function (k , v) {
        ctype_content += '<a href="'+order_car_url+v.car_id+'" class="weui-media-box weui-media-box_appmsg">';
        ...
        ctype_content +=         '<p class="text-gray">日均价</p>';
        ctype_content +=         '<p>';
        ctype_content +=             '<span class="text-gray">￥</span><span class="text-light">'+v.car_rent_price+'</span><span class="text-gray">起</span>';
        ctype_content +=         '</p>';
        ctype_content +=     '</span>';
        ctype_content += '</a>';
    });
