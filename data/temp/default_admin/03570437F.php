...
<?php endif;?><div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody> <?php if (empty($list)):?> <tr>
                                    ...
                                    </tr> <?php endif;?><?php unset($k, $v); $k_index = 0; foreach ((array)$list as $k => $v): $k_index ++;?> <tr id="item-<?php echo($v['user_id']);?>">
                                        <td>
                                            <input type="checkbox" class="i-checks" value="<?php echo($v['user_id']);?>" />
                                        </td>
                                        <td>
                                            <a title="编辑<?php echo($v['user_name']);?>" href="<?php echo(RGX\router::url('user-add-id-%d' , $v['user_id'])); ?>"><?php echo($v['user_sno']);?></a>
                                        </td>
                                        <td> <?php echo($v['user_name']);?> </td>
                                        <td> <?php echo($v['user_sname']);?> </td>
                                        <td><?php echo($v['user_mobile']);?></td>
                                        <td>
                                            <a href="<?php echo(RGX\router::url('user-del-id-%d' , $v['user_id'])); ?>" data-confirm="操作不可撤销，确认吗？" data-target="#item-<?php echo($v['user_id']);?>" class="btn btn-xs btn-danger btn-delete">
