...
<?php endif;?><div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody> <?php if (empty($list)):?> <tr>
                                    ...
                                    </tr> <?php endif;?><?php unset($k, $v); $k_index = 0; foreach ((array)$list as $k => $v): $k_index ++;?> <tr id="item-<?php echo($v['order_id']);?>">
                                        <td>
                                            <input type="checkbox" class="i-checks" value="<?php echo($v['order_id']);?>" />
                                        </td>

                                        <td>
                                            <a title="编辑<?php echo($v['order_title']);?>" href="<?php echo(RGX\router::url('order-add-id-%d' , $v['order_id'])); ?>"><?php echo($v['order_no']);?></a>
                                        </td>
                                        <td><?php echo($v['order_title']);?></td>
                                        <td class="text-warning">￥<?php echo($v['order_amount']);?></td>
                                        <td>
                                            <p>租车人：<?php echo($v['order_user_fullname']);?></p>
                                            <p>手机号：<?php echo($v['order_user_mobile']);?></p>
...
<script>
    ...
    <script type="text/javascript">
        $('.order-operate a').on('click' , function() {
            ...
            var url = "<?php echo(RGX\router::url('order-change-status-#SAT#-id-~')); ?>" + order_id;
